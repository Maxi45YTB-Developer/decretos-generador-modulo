...
def generar_documento_formato7(lista_licencias: list, datos_extra: dict, isapre: str):
    """
    Formato 7: “Múltiple ISAPRE (sin subrogancia)”
    ...
    - datos_extra: dict con llaves:
        decreto_aut_excel, fecha_decreto_excel, secretario, resoluciones (dict id→resolución)
    - isapre: nombre de la ISAPRE elegida
    """

    plantilla_path = os.path.join("templates", "7 MULTIPLE ISAPRE.docx")
    ...
    for lic in lista_licencias:
        ...
        if decreto_excel and fecha_decreto_excel:
            vistos_rt.add(
        ...
        letra = chr(ord('a') + letra_ord)
        vistos_rt.add(f"{letra}) ", bold=True, font="Arial", size=22)
        num_res = datos_extra["resoluciones"].get(str(int(float(lic["id"]))), "")
        vistos_rt.add(f"Resolución de la Isapre {isapre} N° {num_res} que autoriza Licencia Médica Nº {int(float(lic['id']))}.\n",
                      font="Arial", size=22)
        letra_ord += 1
    ...
    if len(lista_licencias) >= 2:
        vistos_letras = "a y c"
        vistos_isapre_letras = "b y d"
    else:
        vistos_letras = "a"
        vistos_isapre_letras = "b"
    ...
    rt_decreto.add(
        ...
        f"por informes mencionados en los Vistos letra {vistos_isapre_letras} emitidos por ",
        font="Arial", size=22
    )
    rt_decreto.add(isapre, bold=True, font="Arial", size=22)
    ...
    nombre_archivo = f"{anio} D.A. Nº {num_da} de fecha {fecha_da} que autoriza Licencias Médicas múltiples (ISAPRE).docx"
...
def generar_documento_formato8(lista_licencias: list, datos_extra: dict, datos_subrogancia: dict, isapre: str):
    """
    Formato 8: “Subrogancia Múltiple ISAPRE”
    - lista_licencias: lista de diccionarios con datos de varias licencias
    - datos_extra: dict con llaves:
        decreto_aut_excel, fecha_decreto_excel, secretario, resoluciones (id→n°resolución)
    ...
    plantilla_path = os.path.join("templates", "8 SUBROGANTE MULTIPLE ISAPRE.docx")
    doc = DocxTemplate(plantilla_path)
    ...
    rt_decreto.add(isapre, bold=True, font="Arial", size=22)
    rt_decreto.add(
        f" y subrogancia mencionada en la viñeta letra c).",
        font="Arial", size=22
    )
    ...
    nombre_archivo = f"{anio} D.A. Nº {num_da} de fecha {fecha_da} que autoriza Licencias Médicas múltiples (ISAPRE Subrogancia).docx"
    nombre_archivo = nombre_archivo.replace("/", "-").replace(":", "").replace("|", "")
    ruta_salida = os.path.join(carpeta_decretos, nombre_archivo)
    doc.save(ruta_salida)
    return ruta_salida
