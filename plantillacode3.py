...
def generar_documento_formato5(licencia: dict, datos_extra: dict, isapre: str):
    """
    Formato 5: “Solo ISAPRE (sin subrogancia)”
    ...
    - isapre: nombre de la ISAPRE elegida (ej. "Banmédica")
    """

    plantilla_path = os.path.join("templates", "5 SOLO ISAPRE.docx")
    ...
    rt_v_b.add(f"Resolución de la Isapre {isapre} N° ", font="Arial", size=22)
    rt_v_b.add("[NÚMERO_RESOLUCIÓN]", bold=True, font="Arial", size=22)
    rt_v_b.add(f" que autoriza Licencia Médica Nº {id_lic}.\n", font="Arial", size=22)
    ...
    rt_decreto.add(
        f", Rut {licencia.get('rut_titular','')}, {licencia.get('escalafon','')}, grado {int(float(licencia.get('grado_raw',0)))}° de la escala municipal, por resolución de la Isapre {isapre}.\n",
        font="Arial", size=22
    )
    ...
    contexto = {
        ...
        "ROL_TITULAR": rol_tit,
        "ISAPRE": isapre,
        "NUM_RESOLUCION": "[NÚMERO_RESOLUCIÓN]"
    }
    ...
    nombre_archivo = f"{anio} D.A. Nº {num_da} de fecha {fecha_da} que autoriza Licencia Médica de {licencia['nombre_titular']} (ISAPRE).docx"
...
def generar_documento_formato6(licencia: dict, datos_extra: dict, datos_subrogancia: dict, isapre: str):
    """
    Formato 6: “Subrogancia Solo ISAPRE”
    - licencia: diccionario con llaves:
        id, nombre_titular, rut_titular, escalafon, grado_raw,
        periodo_inicio, periodo_fin, decreto_aut_excel, fecha_decreto_excel, genero, dias
    - datos_extra: dict con llaves:
    ...
    plantilla_path = os.path.join("templates", "6 SUBROGANTE SOLO ISAPRE.docx")
    doc = DocxTemplate(plantilla_path)
    ...
    rt_decreto.add(licencia["nombre_titular"], bold=True, font="Arial", size=22)
    rt_decreto.add(
        f", Rut {licencia.get('rut_titular','')}, {licencia.get('escalafon','')}, grado {int(float(licencia.get('grado_raw',0)))}° de la escala municipal, por resolución de la Isapre {isapre} y subrogancia mencionada en la viñeta letra c).",
        font="Arial", size=22
    )
    ...
    nombre_archivo = f"{anio} D.A. Nº {num_da} de fecha {fecha_da} que autoriza Licencia Médica de {licencia['nombre_titular']} (ISAPRE Subrogancia).docx"
    nombre_archivo = nombre_archivo.replace("/", "-").replace(":", "").replace("|", "")
    ruta_salida = os.path.join(carpeta_decretos, nombre_archivo)
    doc.save(ruta_salida)
    return ruta_salida
